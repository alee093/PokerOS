// src/pages/tournaments/components/TournamentForm.tsx
...
export default function TournamentForm({
  ...
  const [entries, setEntries] = useState(String(initialValues?.entries ?? 1));
  const [buyIn, setBuyIn] = useState(String(initialValues?.buyIn ?? ""));
  const [fee, setFee] = useState(String(initialValues?.fee ?? ""));
  const [isBounty, setIsBounty] = useState(initialValues?.isBounty ?? false);
  const [bountyCollected, setBountyCollected] = useState(
    String(initialValues?.bountyCollected ?? 0)
  );
  const [prize, setPrize] = useState(String(initialValues?.prize ?? 0));
  ...
  const [startedAt, setStartedAt] = useState(
    initialValues?.startedAt ? initialValues.startedAt.slice(0, 16) : ""
  );
  const [finishedAt, setFinishedAt] = useState(
    initialValues?.finishedAt ? initialValues.finishedAt.slice(0, 16) : ""
  );
  ...
  useEffect(() => {
    async function loadSites() {
      try {
        ...
        setSitesError("Could not load poker sites");
      } finally {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    await onSubmit({
      ...
      ...(playersCount && { playersCount: Number(playersCount) }),
    ...
    <form className="tournament-form" onSubmit={handleSubmit}>
      <div className="tournament-form__grid">
        <div className="form-field">
          <label htmlFor="siteId">Poker Site</label>

          {sitesLoading ? (
            <p className="dashboard-card__empty">Loading poker sites...</p>
          ) : sitesError ? (
            <p className="auth-form__error">{sitesError}</p>
          ) : sites.length === 0 ? (
            <p className="dashboard-card__empty">No poker sites available.</p>
          ) : (
            <select
              id="siteId"
              value={siteId}
        ...
        <div className="form-field">
          ...
          <select
            ...
            value={speed}
            onChange={(event) =>
              setSpeed(event.target.value as TournamentSpeed)
            }
          >
      ...
      <label className="tournament-form__checkbox">
        <input
          type="checkbox"
          checked={isBounty}
          onChange={(event) => setIsBounty(event.target.checked)}
        />
        Bounty tournament
      </label>
      ...
      {isBounty && (
        <div className="form-field">
          <label htmlFor="bountyCollected">Bounty collected</label>
          <input
            id="bountyCollected"
            type="number"
            min="0"
            ...
            onChange={(event) => setBountyCollected(event.target.value)}
          />
        </div>
      )}
